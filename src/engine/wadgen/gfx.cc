...
void Gfx_CreateEXLump(gfxEx_t * pcGfx, gfxRom_t * romGfx)
{
	...
	pcGfx->data = (byte*) malloc(pcGfx->width * pcGfx->height);
...
void Gfx_GetCloudLump(gfxEx_t * pcGfx)
{
	...
	romWadFile.lumpcache[l] = (byte*) Wad_GetLump(romWadFile.lump[l].name, false);

	ZeroMemory(pcGfx, sizeof(gfxEx_t));

	pcGfx->width = 64;
	pcGfx->height = 64;
	pcGfx->data = (byte*) malloc(4096);
	...
	for (i = 0; GfxNames[i].name; i++) {
		...
		if (!strncmp("CLOUD", GfxNames[i].name, 8))
			...
			romWadFile.lumpcache[l] =
			    (byte*) Wad_GetLump(romWadFile.lump[l].name, false);
