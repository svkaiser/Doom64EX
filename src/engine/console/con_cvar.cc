...
void CON_CvarAutoComplete(char *partial) {
    ...
    for(cvar = cvarcap; cvar; cvar = cvar->next) {
        if(!dstrncmp(partial, cvar->name, len)) {
            ...
            spacing = (char *) Z_Malloc(spacinglength + 1, PU_STATIC, NULL);
...
void CON_CvarSet(const char *var_name, char *value) {
    ...
    var->string = (char *) Z_Malloc(dstrlen(value)+1, PU_STATIC, 0);
...
void CON_CvarRegister(cvar_t *variable) {
    ...
    variable->string = (char *) Z_Malloc(dstrlen(variable->string)+1, PU_STATIC, 0);
    dstrcpy(variable->string, oldstr);
    variable->value = datof(variable->string);
    variable->defvalue = (char *) Z_Malloc(dstrlen(variable->string)+1, PU_STATIC, 0);
