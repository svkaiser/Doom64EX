...
static void P_InitAnimdef(void) {
    ...
    while(sc_parser.readtokens()) {
        ...
        if(!dstricmp(sc_parser.token, "ANIMPIC")) {
            ...
            animdefs = (animdef_t*) Z_Realloc(animdefs,
...
void P_SpawnDelayTimer(line_t* line, void (*func)(void)) {
    ...
    timer = (delay_t*) Z_Malloc(sizeof(*timer), PU_LEVSPEC, 0);
...
static void P_SpawnQuake(void *data) {
    ...
    quake = (quake_t*) Z_Malloc(sizeof(*quake), PU_LEVSPEC, 0);
...
int P_SetAimCamera(player_t* player, line_t* line, dboolean aim) {
    ...
    for(mo = mobjhead.next; mo != &mobjhead; mo = mo->next) {
        ...
        camera = (aimcamera_t*) Z_Malloc(sizeof(*camera), PU_LEVSPEC, 0);
...
void P_SetMovingCamera(player_t* player, line_t* line) {
    ...
    camera = (movecamera_t*) Z_Malloc(sizeof(*camera), PU_LEVSPEC, 0);
    P_AddThinker(&camera->thinker);
...
void P_SpawnSpecials(void) {
    ...
    //    Init line EFFECTs
    numlinespecials = 0;
    linespeciallist = (line_t**) Z_Malloc(sizeof(line_t*) * 1, PU_LEVEL, 0);
    for(i = 0; i < numlines; i++) {
