...
static void SetupLists(void) {
    ...
    if(!SetupList(&iwad_gfx, &iwad, "G_START", "G_END", NULL, NULL)) {
        I_Error("GFX section not found in IWAD");
    }

    if(!SetupList(&iwad_sounds, &iwad, "DS_START", "DS_END", NULL, NULL)) {
    ...
    SetupList(&pwad_gfx,        &pwad, "G_START", "G_END", "GG_START", "GG_END");
    SetupList(&pwad_sounds,     &pwad, "DS_START", "DS_END", NULL, NULL);
}
...
static void DoMerge(void) {
    ...
    for(i = 0; i < iwad.numlumps; ++i) {
        ...
        case SECTION_NORMAL:
            ...
            else if(!strncasecmp(lump->name, "G_START", 8)) {
                current_section = SECTION_GFX;
            }
            else if(!strncasecmp(lump->name, "DS_START", 8)) {
        ...
        case SECTION_GFX:
            ...
            if(!strncasecmp(lump->name, "G_END", 8)) {
                // Add all new flats from the PWAD to the end
                // of the section
        ...
        case SECTION_SOUNDS:
            ...
            if(!strncasecmp(lump->name, "DS_END", 8)) {
                // Add all new flats from the PWAD to the end
                // of the section
    ...
    for(i = 0; i < pwad.numlumps; ++i) {
        ...
        case SECTION_NORMAL:
            if(!strncasecmp(lump->name, "T_START", 8)
                ...
                current_section = SECTION_SPRITES;
                    ...
                    || !strncasecmp(lump->name, "GG_START", 8)) {
                current_section = SECTION_GFX;
            }
            else if(!strncasecmp(lump->name, "DS_START", 8)) {
                current_section = SECTION_SOUNDS;
        ...
        case SECTION_GFX:
            ...
            if(!strncasecmp(lump->name, "GG_END", 8)
                    || !strncasecmp(lump->name, "G_END", 8)) {
                // end of section
                current_section = SECTION_NORMAL;
            }
            break;
